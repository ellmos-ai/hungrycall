...
```powershell
python -m hungrycall.cli delivery --food "2x Döner Kebab" --address "Dorfstrasse 1, 12345 Dorfstadt" --budget 30.0 --customer-name "Alex" --scenario success_direct
```
...
Attempt History & Activity Stream:
  ...
  Attempt #2: Burger House Dorfstadt (+491 ••• ••••000) -> [PASSED]
    Live Activity Progress:
      ...
      * 17:37:50.700 | Bot is speaking: Hello, I am an automated assistant calling on behalf of Alex. Do you deliver to Dorfstrasse 1, 12345 Dorfstadt?
...
Attempt History & Activity Stream:
  ...
  Attempt #1: Burger House Dorfstadt (+491 ••• ••••000) -> [REJECTED]
    ...
    Live Activity Progress:
      ...
      * 17:37:44.200 | Call is ringing (~40s setup latency).
      * 17:37:49.500 | Call connected.
      * 17:37:50.700 | Bot is speaking: Hello, calling on behalf of Alex. What is the total price for delivery of Burger?
  ...
  Attempt #2: Trattoria Bella Luigi (+491 ••• ••••001) -> [PASSED]
    Live Activity Progress:
      ...
      * 17:38:56.700 | Bot is speaking: Hello, calling on behalf of Alex.
...
Attempt History & Activity Stream:
  ...
  Attempt #1: Trattoria Bella Luigi (+491 ••• ••••001) -> [REJECTED]
    ...
    Live Activity Progress:
      ...
      * 17:38:55.700 | Bot is speaking: Hello, calling on behalf of Alex. What is the total price for delivery of 2x Döner Kebab & Drinks?
  ...
  Attempt #2: Burger House Dorfstadt (+491 ••• ••••000) -> [REJECTED]
    ...
    Live Activity Progress:
      ...
      * 17:37:49.500 | Call connected.
      * 17:37:50.700 | Bot is speaking: Hello, calling on behalf of Alex. What is the total price for delivery of 2x Döner Kebab & Drinks?
      * 17:37:52.200 | Callee said: 42 Euro.
      * 17:38:00.100 | Call ended; syncing final Calling result.
  ...
  Attempt #3: Asia Wok Express (+491 ••• ••••002) -> [PASSED]
    Live Activity Progress:
      ...
      * 17:40:00.700 | Bot is speaking: Hello, calling on behalf of Alex. Do you deliver to Dorfstrasse 1, 12345 Dorfstadt?
...
Verification Transcript (Order Proof):
  ...
  [00:40] BOT: Thank you. Callback at +44 7700 900002.
============================================================
```
...
goal starts: Hello, I am an automated assistant calling on behalf of Alex. We would like to order food
...
## 16. Supervised live field trial (2026-08-11; privacy-safe record)

> **Publication boundary:** Real CALL-E calls were executed, but this public file does not
> contain a raw or lightly redacted call transcript. It publishes only aggregate findings
> that can be tied to code changes, plus clearly labelled synthetic reproductions of the
> tested dialogue shapes. Call/order identifiers, personal names, exact timestamps and all
> fragments of the consenting test number are omitted.

### 16.1 What was actually executed

- Four supervised live cascades used the real CALL-E REST path (`POST /v1/calls`).
- `HUNGRYCALL_FIELD_TRIAL_PHONE` redirected every candidate to one explicitly
  consenting test line. The public fixture restaurant set supplied names and scenarios;
  no real restaurant number was dialled.
- A human operator answered the consenting line and role-played the counterpart.
- Local, gitignored receipts were used during diagnosis. Those receipts and their raw
  transcripts are not public evidence artifacts and are not reproduced here.
- The trial covered delivery with an item fallback, pickup with a vague price,
  reservation with a newly introduced fee, and a later delivery success path.

### 16.2 Findings demonstrated by the field trial

1. **Recipient-envelope selection:** CALL-E returned a sparse top-level structured result
   and a populated recipient result. Choosing the top level first caused false negatives;
   commit `ff7f5ce` makes recipient data authoritative.
2. **Vague-price gate:** an approximate amount was rejected until an exact amount and
   pickup time were supplied.
3. **Independent reservation authority:** a fee outside the configured zero-euro
   authority was rejected by the local evaluator even though the voice agent had
   verbally accepted it. This produced the retract-aloud correction.
4. **Idempotent create retry:** one timed-out create request still caused a live call.
   Commit `1b9883f` retries with the same idempotency key so a retry re-attaches.
5. **Decimal handling:** the later delivery trial preserved a spoken decimal amount.
6. **Quantity verification gap:** the prompt requested two fallback items, but the
   spoken closeout placed one. The pre-fix evaluator did not compare ordered quantity;
   commit `7427c85` adds that structural check.

### 16.3 Synthetic reproductions of the tested shapes

The following snippets are **synthetic reproductions**, not quotations and not
transcript fragments. They demonstrate the conditions exercised without publishing
a person's words.

```text
[SYNTHETIC — vague-price gate]
COUNTERPART: The total is approximately twenty euros.
AGENT: I need the exact total and pickup time before I can place the order.
COUNTERPART: The exact total is seventeen euros; pickup is at 20:45.
```

```text
[SYNTHETIC — fee outside authority]
COUNTERPART: The reservation has an additional five-euro fee.
AGENT: That exceeds my authorised fee limit, so I must cancel the reservation.
```

```text
[SYNTHETIC — quantity gap found by the trial]
PLAN: Order two units of the accepted fallback item.
AGENT CLOSEOUT: Please confirm one unit of the fallback item.
AUDIT: Reject because the confirmed quantity differs from the configured quantity.
```

### 16.4 Evidence limits

- The fact that real service traffic ran is preserved; the public conversation examples
  are synthetic and cannot authenticate anyone's exact spoken wording.
- No independent audio recording exists. The live diagnosis used provider transcripts
  and local evaluation records available to the operator at the time.
- Mailbox/call-screening behaviour, balance observations and provider-side retries were
  operator-observed where the repository has no durable independent artifact.
- A masked display value is still contact data if it preserves identifying fragments.
  Therefore this revision publishes no fragment of the consenting test line.
- Public examples elsewhere in the repository use only German numbers reserved for
  media productions by Bundesnetzagentur Mitteilung 148/2021.
