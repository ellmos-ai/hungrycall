#!/usr/bin/env python3
"""Package, deploy, and tear down the ``calle-demo-hungrycall`` Lambda Function URL.
...
project's only storage is stdlib SQLite (``hungrycall/store.py``), so there is no
...
``package``      Build ``calle-demo-hungrycall.zip``: the whole ``hungrycall``
...
2. ``DEMO_MODE=1`` is what actually blocks a call, at the code layer:
   ``hungrycall.transports.calle.CalleTransport.__init__`` checks it *before*
...
PACKAGE_DIR = REPO_ROOT / "hungrycall"
INFRA_DIR = Path(__file__).resolve().parent
BUILD_DIR = INFRA_DIR / "build"
DEFAULT_ZIP_PATH = BUILD_DIR / "calle-demo-hungrycall.zip"

FUNCTION_NAME = "calle-demo-hungrycall"
ROLE_NAME = "calle-demo-hungrycall-role"
HANDLER = "demo.lambda_entry.handler"
DEFAULT_REGION = "eu-central-1"

# mangum has no third-party dependencies of its own; fastapi pulls in
# starlette, pydantic, pydantic-core, anyio, sniffio, idna,
# typing-extensions and annotated-types transitively. python-multipart is
# what FastAPI's Form(...) parameters in hungrycall/web.py need. Matches
# the `lambda` extra in pyproject.toml -- deliberately no uvicorn (Mangum
# replaces it), no jinja2 (hungrycall/templates.py builds HTML itself) and
# no database driver (SQLite is stdlib). httpx is not optional despite
# this deployment never reaching out: hungrycall/location.py imports it at
# module scope, so leaving it out makes the whole app fail to import (a
# 502 on every request, which is exactly what happened on the first
# deploy attempt). What keeps the demo from actually querying a geo
# service is the restaurant fixture mode demo/lambda_entry.py defaults a
# visitor into, not the absence of an HTTP client.
...
def cmd_package(args: argparse.Namespace) -> None:
    ...
    print(f"Copying {PACKAGE_DIR} -> {build_dir / 'hungrycall'}")
    _copy_source_tree(PACKAGE_DIR, build_dir / "hungrycall")
...
LOG_GROUP_POLICY_NAME = "calle-demo-hungrycall-logs"
...
def cmd_create_role(args: argparse.Namespace) -> None:
    ...
    try:
        role = iam.create_role(
            ...
            Description="Execution role for the calle-demo-hungrycall Lambda "
...
def cmd_deploy(args: argparse.Namespace) -> None:
    ...
    try:
        response = lam.create_function(
            ...
            Description="hungrycall public dry-run demo (CALL-E hackathon). "
...
def _wait_for_update_settled(lam: Any, function_name: str, *, attempts: int = 10) -> None:
    ...
    Reads the state through `get_function` rather than the more obvious
    `get_function_configuration`: the deploy user's policy grants
    `lambda:GetFunction` only, and `GetFunctionConfiguration` is a separate IAM
    action. `get_function` returns the same information under `Configuration`,
    so this stays inside the least-privilege policy instead of asking for it to
    be widened."""
    for _ in range(attempts):
        state = lam.get_function(FunctionName=function_name).get("Configuration", {})
...
def cmd_teardown(args: argparse.Namespace) -> None:
    ...
    print(
        ...
        "AWS console that no calle-demo-hungrycall resources remain billable"
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Package, deploy, and tear down the calle-demo-hungrycall "
        "Lambda Function URL."
    )
